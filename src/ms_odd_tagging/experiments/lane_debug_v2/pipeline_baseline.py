...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--fg);font:14px system-ui,sans-serif}main{max-width:1280px;margin:auto;padding:18px}.toolbar{display:flex;gap:12px;align-items:center;flex-wrap:wrap}.toolbar input[type=range]{flex:1;min-width:260px}.toolbar input[type=number]{width:90px;padding:6px}button{padding:7px 12px}canvas{display:block;width:100%;background:var(--panel);border:1px solid var(--border);border-radius:10px}.map{margin-top:12px}.timeline{margin-top:12px;height:84px}.legend{display:flex;gap:14px;flex-wrap:wrap;margin:10px 0;color:var(--muted)}.sw{display:inline-block;width:12px;height:12px;margin-right:5px;border-radius:3px}.details,.roles{margin-top:12px;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.details>div,.role-card,details{padding:12px;border:1px solid var(--border);border-radius:10px;background:var(--panel)}.details b,.role-card b{display:block;color:var(--muted);font-weight:500;margin-bottom:3px}.role-card{border-top:4px solid var(--role)}.role-card code{display:block;margin:5px 0;white-space:pre-wrap}.candidate{padding:4px 0;border-top:1px solid var(--border);font:12px ui-monospace,monospace}.candidate:first-child{border-top:0}.muted{color:var(--muted)}.state{font-weight:600}code,pre{font-family:ui-monospace,monospace}pre{white-space:pre-wrap;word-break:break-word;font-size:12px;max-height:360px;overflow:auto}h1{font-size:20px;margin:0 0 12px}summary{cursor:pointer;font-weight:600}
</style></head><body><main><h1 id="title"></h1>
<div class="toolbar"><button id="play" type="button">Play</button><button id="prev" type="button">Previous frame</button><button id="next" type="button">Next frame</button><label><input id="showRawLd" type="checkbox"> Show all source LD geometry</label><label for="frameNumber">Go to frame</label><input id="frameNumber" type="number" min="0" step="1"><label for="frame">Frame <span id="frameLabel"></span></label><input id="frame" type="range" min="0" value="0" step="1"></div>
...
<section class="roles" aria-label="Lane assignment decisions">
<article class="role-card" style="--role:var(--left)"><b>Left lane decision</b><code id="leftDecision"></code><div id="leftCandidates"></div></article>
<article class="role-card" style="--role:var(--ego)"><b>Ego lane decision</b><code id="egoDecision"></code><div id="egoCandidates"></div></article>
<article class="role-card" style="--role:var(--right)"><b>Right lane decision</b><code id="rightDecision"></code><div id="rightCandidates"></div></article>
</section>
<details><summary>Complete current-frame detector output</summary><pre id="frameJson"></pre></details>
<script id="payload" type="application/json">__PAYLOAD__</script><script>
const data=JSON.parse(document.getElementById('payload').textContent), map=document.getElementById('map'), ctx=map.getContext('2d'), timeline=document.getElementById('timeline'), tx=timeline.getContext('2d'), slider=document.getElementById('frame'),frameNumber=document.getElementById('frameNumber'),showRawLd=document.getElementById('showRawLd');
...
const frameIndexToPosition=new Map(data.frames.map((frame,index)=>[frame.frame_index,index]));
function value(value,fallback='—'){return value===null||value===undefined||value===''?fallback:String(value)}
function decisionText(assignment){return `physical: ${value(assignment&&assignment.lane_id)}\nlogical: ${value(assignment&&assignment.logical_lane_id)}\nconfidence: ${value(assignment&&assignment.confidence,'unknown')}\nmethod: ${value(assignment&&assignment.method,'unknown')}`}
function renderDecision(role,assignment){document.getElementById(role+'Decision').textContent=decisionText(assignment);const target=document.getElementById(role+'Candidates'),candidates=(assignment&&assignment.candidates)||[];target.replaceChildren();if(!candidates.length){const empty=document.createElement('span');empty.className='muted';empty.textContent=role==='ego'?'No scored candidates reported.':'Adjacent selection does not emit a scored candidate list.';target.append(empty);return}for(const candidate of candidates){const row=document.createElement('div');row.className='candidate';row.textContent=`${value(candidate.lane_id)} · score ${value(candidate.score)} · ${candidate.inside_polygon?'inside polygon':'outside polygon'} · ${value(candidate.boundary_reliability,'reliability unknown')}`;target.append(row)}}
function selectSourceFrame(frameIndex){const position=frameIndexToPosition.get(Number(frameIndex));if(position===undefined){frameNumber.setCustomValidity('Frame is not present in this recording');frameNumber.reportValidity();return}frameNumber.setCustomValidity('');stop();slider.value=position;draw()}
...
slider.setAttribute('aria-valuetext','frame '+f.frame_index);frameNumber.value=f.frame_index;history.replaceState(null,'','#frame='+f.frame_index);document.getElementById('frameLabel').textContent=f.frame_index+' / '+data.frames.at(-1).frame_index;document.getElementById('state').textContent=f.state;document.getElementById('motion').textContent=f.time_since_start_s.toFixed(2)+' s · '+(f.speed_mps==null?'invalid':f.speed_mps.toFixed(2)+' m/s');document.getElementById('lanes').textContent='left '+(f.left_lane.lane_id??'—')+' / '+(f.left_lane.logical_lane_id??'—')+' · ego '+(f.ego_lane.lane_id??'—')+' / '+(f.ego_lane.logical_lane_id??'—')+' · right '+(f.right_lane.lane_id??'—')+' / '+(f.right_lane.logical_lane_id??'—');document.getElementById('lead').textContent=f.lead?'#'+f.lead.object_id+' '+f.lead.class+' at '+f.lead.longitudinal_m.toFixed(1)+' m · '+(f.lead.ego_lane_area_source??'invalid area')+' · '+(f.lead.tracking_status??'untracked'):'none';document.getElementById('evidence').textContent=f.reason+' · '+(f.ego_lane.confidence??'unknown')+' / '+(f.ego_lane.method??'unknown')+' · drivable '+(f.ego_lane.drivable_status??'unknown')+(f.ego_lane.nearest_boundary_normalized_as_lane_line?' · nearest boundary: drivable road boundary normalized as lane line':'')+(f.ego_lane.intersection_connector?' · intersection connector':'');renderDecision('left',f.left_lane);renderDecision('ego',f.ego_lane);renderDecision('right',f.right_lane);document.getElementById('frameJson').textContent=JSON.stringify(f,null,2);drawTimeline(+slider.value)}
...
let timer=null;const play=document.getElementById('play');function stop(){if(timer!==null){clearInterval(timer);timer=null}play.textContent='Play';play.setAttribute('aria-pressed','false')}function togglePlay(){if(timer!==null){stop();return}play.textContent='Pause';play.setAttribute('aria-pressed','true');timer=setInterval(()=>{if(+slider.value>=data.frames.length-1){stop();return}slider.value=+slider.value+1;draw()},100)}slider.addEventListener('input',()=>{stop();draw()});frameNumber.addEventListener('change',()=>selectSourceFrame(frameNumber.value));showRawLd.addEventListener('change',draw);play.addEventListener('click',togglePlay);document.getElementById('prev').onclick=()=>{stop();slider.value=Math.max(0,+slider.value-1);draw()};document.getElementById('next').onclick=()=>{stop();slider.value=Math.min(data.frames.length-1,+slider.value+1);draw()};document.addEventListener('keydown',event=>{if(event.target.matches('input'))return;if(event.key==='ArrowLeft')document.getElementById('prev').click();if(event.key==='ArrowRight')document.getElementById('next').click();if(event.key===' ') {event.preventDefault();togglePlay()}});const requestedFrame=new URLSearchParams(location.hash.slice(1)).get('frame');if(requestedFrame!==null&&frameIndexToPosition.has(Number(requestedFrame)))slider.value=frameIndexToPosition.get(Number(requestedFrame));draw();
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional directory containing original *_animated_odld_explorer.html "
            "files. Omit it to generate the standalone lane debugger."
        ),
    )
