...
def assign_point_to_track_strict(
    ...
    *,
    ...
    for track in tracks:
        ...
        if best_match["piece_kind"] in {"anchored_ld_bridge","canonical_track_stitch","topology_supported_curvature_stitch","ego_supported_inferred_route","static_inferred_corridor"}: score+=0.15
    ...
    if not candidates:
        return {"track_id":None,"logical_lane_id":None,"method":"no_track_contains_ego_center","confidence":"unknown","outside_tolerance_m":outside_tolerance_m,"candidates":[],"rejected_candidates":rejected[:16]}
    best=candidates[0]; margin=candidates[1][0]-best[0] if len(candidates)>1 else None; match=best[2]
    confidence="high" if match["inside_polygon"] and (margin is None or margin>=1.0) else "medium" if match["inside_polygon"] else "low"
    return {
        "track_id":best[1].get("track_id"),"logical_lane_id":best[1].get("logical_lane_id"),"member_lane_ids":best[1].get("member_lane_ids",[]),"method":best[5],"confidence":confidence,
        "inside_polygon":match["inside_polygon"],"polygon_distance_m":round(match["polygon_distance_m"],3),"matched_piece_kind":match["piece_kind"],"matched_lane_id":match.get("lane_id"),"matched_source_lane_id":match.get("source_lane_id"),"matched_destination_lane_id":match.get("destination_lane_id"),
        "outside_tolerance_m":outside_tolerance_m,"center_distance_m":round(best[3],3),"heading_difference_deg":round(best[4],2),"runner_up_score_margin":None if margin is None else round(margin,3),
        "candidates":[{"track_id":item[1].get("track_id"),"score":round(item[0],3),"method":item[5],"inside_polygon":item[2]["inside_polygon"],"polygon_distance_m":round(item[2]["polygon_distance_m"],3),"matched_piece_kind":item[2]["piece_kind"],"matched_lane_id":item[2].get("lane_id"),"center_distance_m":round(item[3],3),"heading_difference_deg":round(item[4],2)} for item in candidates[:5]],
