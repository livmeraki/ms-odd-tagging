...
def run_one(canonical_path: Path, output_root: Path, config: dict[str,Any], lane_change_config: dict[str,Any], run_id: str | None=None) -> list[Path]:
    ...
    _write(lane_path,following); _write(tag_path,changes)
    metadata={
        "run_id":run_id,"run_timestamp_utc":datetime.now(timezone.utc).isoformat(),
        "git_commit_sha":_git_sha(),"input_recording":rid,"input_path":str(canonical_path),
        "debug_implementation_version":DEBUG_IMPLEMENTATION_VERSION,
        "lane_debug_config":config,"lane_change_config_source":"provided direct_scenarios config",
...
def main(argv: list[str]|None=None)->int:
    ...
    if not canonical.is_file():p.error(f"missing canonical recording: {canonical}")
    for path in run_one(canonical,a.output_root,_load(a.config),_load(a.lane_change_config),a.run_id):print(f"Wrote {path}")
    return 0
