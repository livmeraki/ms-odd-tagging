...
<label><input id="inferred" type="checkbox" checked>inferred route boxes</label>
...
function drawUnpromotedInferred(out,f){if(!document.getElementById('inferred').checked)return;const c=f.inferred_ego_corridor||{},rid=c.inferred_ego_route&&c.inferred_ego_route.route_id;if(!rid||promotedRouteIds.has(String(rid)))return;const r=routeMap.get(String(rid));if(!r)return;for(const p of r.pieces||[]){if(p.frame_index>f.frame_index)continue;if((p.polygon_lcs_m||[]).length)out.push(polygonTrace(p.polygon_lcs_m,`inferred route box ${rid} · frame ${p.frame_index}`,colors.ego,1.1,'0f','solid'));}}
