"""Plotly explorer for integrated lane-debug-v2 geometry and static lane ordering."""
...
def render_plotly_explorer(
    ...
    payload = {
        ...
        "topology_stitch_debug": following.get("topology_supported_stitch_debug", []),
...
const promotedRouteIds=new Set(D.tracks.flatMap(t=>promotedRoutePieces(t).map(p=>String(p.route_id))));
...
function drawSmoothFills(out,t,color,strong=false){for(const p of smoothFillPieces(t)){const e=p.connection_evidence||{};const name=`smooth lane completion · ${t.track_id} · ${p.kind} · gap=${Number(e.endpoint_gap_m||0).toFixed(2)}m`;if((p.polygon_lcs_m||[]).length)out.push(polygonTrace(p.polygon_lcs_m,name,color,strong?2:1,strong?'20':'0b','dot'));if((p.centerline_lcs_m||[]).length)out.push(lineTrace(p.centerline_lcs_m,`${name} centerline`,color,strong?1.5:1,'dot'));}}
function drawPromotedRoute(out,t,color,strong=false){for(const p of promotedRoutePieces(t)){if(!(p.polygon_lcs_m||[]).length)continue;out.push(polygonTrace(p.polygon_lcs_m,`ego-supported integrated gap · ${t.track_id} · ${p.route_id}`,color,0,strong?'12':'06','solid'));}}

function drawTrack(out,t,role,strong,constructionOnly=false){
  const color=constructionOnly?colors.irrelevant:(colors[role]||colors.irrelevant);
  for(const id of t.member_lane_ids||[]){const lane=laneMap.get(String(id));if(!lane)continue;const rt=laneRangeText(id),hover=`${constructionOnly?'constructed':role} ${t.track_id} lane ${id} · ${laneRangeHover(id)}`;out.push(polygonTrace(lane.polygon_lcs_m,hover,color,strong?2:0.7,strong?'22':'07'));out.push(lineTrace(lane.left_boundary_lcs_m,`${id} left · ${rt}`,color,strong?1.6:0.6));out.push(lineTrace(lane.right_boundary_lcs_m,`${id} right · ${rt}`,color,strong?1.6:0.6));if(document.getElementById('ids').checked&&(lane.centerline_lcs_m||[]).length){const q=lane.centerline_lcs_m[Math.floor(lane.centerline_lcs_m.length/2)];out.push({x:[q[0]],y:[q[1]],mode:'text',text:[`${id} · ${rt}`],textposition:'middle center',showlegend:false,hovertext:[hover],hoverinfo:'text',textfont:{size:10}});}}
  drawSmoothFills(out,t,color,strong);drawPromotedRoute(out,t,color,strong);if(strong)drawAnchored(out,t,color,true);
  if(document.getElementById('ids').checked&&(t.centerline_lcs_m||[]).length){const q=t.centerline_lcs_m[Math.floor(t.centerline_lcs_m.length/2)];out.push({x:[q[0]],y:[q[1]],mode:'text',text:[t.track_id],textposition:'top center',showlegend:false,textfont:{size:11}});}
}
function roleMap(f){return new Map(((((f.lane_roles||{}).roles)||[])).map(x=>[String(x.track_id),x.role]));}
function drawUnpromotedInferred(out,f){const c=f.inferred_ego_corridor||{},rid=c.inferred_ego_route&&c.inferred_ego_route.route_id;if(!rid||promotedRouteIds.has(String(rid)))return;const r=(D.routes||[]).find(x=>x.route_id===rid);if(!r)return;const p=(r.pieces||[]).find(x=>x.frame_index===f.frame_index);if(p&&(p.polygon_lcs_m||[]).length)out.push(polygonTrace(p.polygon_lcs_m,`unpromoted inferred ego corridor ${rid}`,colors.ego,1,'0d','dot'));}
...
function draw(){
  const f=D.frames[+slider.value]||{},ego=f.ego_position||[0,0],out=[],roles=roleMap(f);
  if(document.getElementById('raw').checked)for(const r of D.raw)out.push(lineTrace(r.pts,`${r.kind} ${r.id}`,'#cbd5e1',0.7));
  if(document.getElementById('canonical').checked)for(const t of D.tracks)drawTrack(out,t,'irrelevant',false,true);
  if(document.getElementById('bridges').checked)for(const t of D.tracks)drawAnchored(out,t,colors.bridge,false);
  if(document.getElementById('selected').checked){for(const t of D.tracks){const role=roles.get(String(t.track_id));if(role&&role!=='irrelevant')drawTrack(out,t,role,true,false);}drawUnpromotedInferred(out,f);}
  drawOrder(out,f);
  ...
  const follow=document.getElementById('follow').checked,xs=span?span.x:110,ys=span?span.y:110;
  const xr=follow?[ego[0]-xs/2,ego[0]+xs/2]:(view?view.x:[ego[0]-55,ego[0]+55]);
  const yr=follow?[ego[1]-ys/2,ego[1]+ys/2]:(view?view.y:[ego[1]-55,ego[1]+55]);
  Plotly.react(plot,out,{margin:{l:35,r:10,t:10,b:35},xaxis:{scaleanchor:'y',scaleratio:1,range:xr},yaxis:{range:yr},uirevision:'integrated-lane-network'},{responsive:true,displaylogo:false}).then(()=>{if(!relayoutBound){plot.on('plotly_relayout',e=>{const x0=e['xaxis.range[0]'],x1=e['xaxis.range[1]'],y0=e['yaxis.range[0]'],y1=e['yaxis.range[1]'];if([x0,x1,y0,y1].every(Number.isFinite)){view={x:[x0,x1],y:[y0,y1]};span={x:Math.abs(x1-x0),y:Math.abs(y1-y0)};}});relayoutBound=true;}}).catch(e=>{console.error('Plotly render failed',e);document.getElementById('panel').textContent=`Plotly render failed: ${e&&e.stack?e.stack:e}`;});
  document.getElementById('label').textContent=`frame ${f.frame_index} · ${Number(f.time_since_start_s||0).toFixed(2)}s`;
  document.getElementById('panel').textContent=JSON.stringify({ego_lane:f.ego_lane,ego_lane_boundary_indices:laneDebug(f.ego_lane),left_lane_boundary_indices:laneDebug(f.left_lane),right_lane_boundary_indices:laneDebug(f.right_lane),lane_roles:f.lane_roles,inferred_ego_corridor:f.inferred_ego_corridor,canonical_track_stitch_debug:D.stitch_debug,topology_supported_stitch_debug:D.topology_stitch_debug,constructed_lane_count:D.network&&D.network.lane_count,canonical_track_count_before_bridge_merge:D.canonical_track_count,anchored_bridge_count:D.bridge_count,anchored_bridge_debug:D.bridge_debug,anchored_bridge_merge_debug:D.bridge_merge_debug,frame_local_adjacency_debug:f.frame_local_adjacency_debug},null,2);
}
