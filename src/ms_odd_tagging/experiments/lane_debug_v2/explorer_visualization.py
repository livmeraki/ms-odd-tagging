...
const D=JSON.parse(document.getElementById('payload').textContent), slider=document.getElementById('frame'); slider.max=D.frames.length-1; document.getElementById('title').textContent=`${{D.recording_id}} — lane debug v2 — run ${{D.run_id}}`;
...
function draw(){{const f=D.frames[+slider.value], traces=[]; const ep=f.ego_position||[0,0];
 ...
 const range=55; Plotly.react('plot',traces,{{margin:{{l:40,r:15,t:15,b:40}},xaxis:{{scaleanchor:'y',scaleratio:1,range:[ep[0]-range,ep[0]+range]}},yaxis:{{range:[ep[1]-range,ep[1]+range]}},hovermode:'closest',uirevision:'lane-debug-v2'}},{{responsive:true,displaylogo:false}});
