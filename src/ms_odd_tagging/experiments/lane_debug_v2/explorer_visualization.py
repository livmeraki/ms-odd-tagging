...
function drawTrack(out,tr,role,strong){{const c=col[role]||col.irrelevant;let rendered=false;for(const id of tr.member_lane_ids||[]){{const l=laneMap.get(String(id));if(!l)continue;rendered=true;out.push(pg(l.polygon_lcs_m,`${{role}} ${{tr.track_id}} lane ${{id}}`,c,strong?2:0.7,strong?'22':'07'));out.push(ln(l.left_boundary_lcs_m,`${{id}} left`,c,strong?1.6:0.6));out.push(ln(l.right_boundary_lcs_m,`${{id}} right`,c,strong?1.6:0.6))}}if(!rendered&&(tr.polygon_lcs_m||[]).length){{out.push(pg(tr.polygon_lcs_m,`${{role}} ${{tr.track_id}} · ${{tr.source||'constructed'}}`,c,strong?2:0.8,strong?'22':'09','dash'));if((tr.left_boundary_lcs_m||[]).length)out.push(ln(tr.left_boundary_lcs_m,`${{tr.track_id}} left raw LD boundary ${{tr.left_boundary_id||''}}`,c,strong?1.8:0.8,'dash'));if((tr.right_boundary_lcs_m||[]).length)out.push(ln(tr.right_boundary_lcs_m,`${{tr.track_id}} right raw LD boundary ${{tr.right_boundary_id||''}}`,c,strong?1.8:0.8,'dash'))}}if(document.getElementById('ids').checked&&(tr.centerline_lcs_m||[]).length){{const q=tr.centerline_lcs_m[Math.floor(tr.centerline_lcs_m.length/2)];out.push({{x:[q[0]],y:[q[1]],mode:'text',text:[tr.track_id],showlegend:false}})}}}}
...
function draw(){{const f=D.frames[+s.value]||{{}},ego=f.ego_position||[0,0],out=[],roles=roleMap(f);if(document.getElementById('raw').checked)for(const r of D.raw)out.push(ln(r.pts,`${{r.kind}} ${{r.id}}`,'#cbd5e1',0.7));if(document.getElementById('allTracks').checked)for(const tr of D.tracks)drawTrack(out,tr,roles.get(String(tr.track_id))||'irrelevant',false);if(document.getElementById('selected').checked)for(const tr of D.tracks){{const role=roles.get(String(tr.track_id));if(role&&role!=='irrelevant')drawTrack(out,tr,role,true)}}
