...
const laneMap=new Map(D.lanes.map(x=>[String(x.lane_id),x])),trackMap=new Map(D.tracks.map(x=>[String(x.track_id),x]));const col={{ego:'#22c55e',left_adjacent:'#06b6d4',right_adjacent:'#f59e0b',irrelevant:'#94a3b8',corridor:'#16a34a'}};let timer=null,view=null,span=null,bound=false;
...
function drawTrack(out,tr,role,strong){{const c=col[role]||col.irrelevant;for(const id of tr.member_lane_ids||[]){{const l=laneMap.get(String(id));if(!l)continue;out.push(pg(l.polygon_lcs_m,`${{role}} ${{tr.track_id}} lane ${{id}}`,c,strong?2:0.7,strong?'22':'07'));out.push(ln(l.left_boundary_lcs_m,`${{id}} left`,c,strong?1.6:0.6));out.push(ln(l.right_boundary_lcs_m,`${{id}} right`,c,strong?1.6:0.6))}}if(document.getElementById('ids').checked&&(tr.centerline_lcs_m||[]).length){{const q=tr.centerline_lcs_m[Math.floor(tr.centerline_lcs_m.length/2)];out.push({{x:[q[0]],y:[q[1]],mode:'text',text:[tr.track_id],showlegend:false}})}}}}
