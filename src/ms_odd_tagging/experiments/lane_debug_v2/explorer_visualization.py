...
function line(pts,name,color,width=2,dash='solid'){{return{{x:(pts||[]).map(p=>p[0]),y:(pts||[]).map(p=>p[1]),mode:'lines',name,line:{{color,width,dash}},showlegend:false,hoverinfo:'name'}}}}function poly(pts,name,color){{return{{x:(pts||[]).map(p=>p[0]),y:(pts||[]).map(p=>p[1]),mode:'lines',name,line:{{color,width:2}},fill:'toself',fillcolor:color+'18',showlegend:false,hoverinfo:'name'}}}}
