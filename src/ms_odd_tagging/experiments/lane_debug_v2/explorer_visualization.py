"""Plotly explorer for the static constructed lane network."""
...
function isRecovered(tr){{return tr.source==='raw_ld_gap_recovery'}}
function drawTrack(out,tr,role,strong,constructionOnly=false){{let c=constructionOnly?(isRecovered(tr)?col.recovered:col.irrelevant):(col[role]||col.irrelevant);let rendered=false;for(const id of tr.member_lane_ids||[]){{const l=laneMap.get(String(id));if(!l)continue;rendered=true;out.push(pg(l.polygon_lcs_m,`${{constructionOnly?'constructed':role}} ${{tr.track_id}} lane ${{id}}`,c,strong?2:0.7,strong?'22':'07'));out.push(ln(l.left_boundary_lcs_m,`${{id}} left`,c,strong?1.6:0.6));out.push(ln(l.right_boundary_lcs_m,`${{id}} right`,c,strong?1.6:0.6))}}if(!rendered&&(tr.polygon_lcs_m||[]).length){{out.push(pg(tr.polygon_lcs_m,`${{constructionOnly?'gap recovered':role}} ${{tr.track_id}}`,c,strong?2:1,strong?'22':'0c','dash'));if((tr.left_boundary_lcs_m||[]).length)out.push(ln(tr.left_boundary_lcs_m,`${{tr.track_id}} left boundary ${{tr.left_boundary_id||''}}`,c,strong?1.8:1,'dash'));if((tr.right_boundary_lcs_m||[]).length)out.push(ln(tr.right_boundary_lcs_m,`${{tr.track_id}} right boundary ${{tr.right_boundary_id||''}}`,c,strong?1.8:1,'dash'))}}if(document.getElementById('ids').checked&&(tr.centerline_lcs_m||[]).length){{const q=tr.centerline_lcs_m[Math.floor(tr.centerline_lcs_m.length/2)];out.push({{x:[q[0]],y:[q[1]],mode:'text',text:[tr.track_id],showlegend:false}})}}}}
