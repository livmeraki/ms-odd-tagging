...
const key='simplified-gt-v2:{recording}'; let rows=JSON.parse(localStorage.getItem(key)||'null')||initialRows; let i=0;
...
function renderBEV(r){{const img=document.getElementById('bevImg'), missing=document.getElementById('bevMissing'), path=document.getElementById('bevPath'); img.style.display='none'; missing.style.display='block'; missing.textContent=`BEV unavailable for frame ${{r.frame_index}}`; path.textContent=r.bev_uri||'No BEV root configured'; if(!r.bev_uri)return; img.onload=()=>{{img.style.display='block';missing.style.display='none';}}; img.onerror=()=>{{img.style.display='none';missing.style.display='block';missing.textContent=`BEV not generated yet for frame ${{r.frame_index}}`;}}; img.src=r.bev_uri+'?frame='+r.frame_index;}}
function render(){{let r=rows[i]; ensureGT(r); document.getElementById('progress').textContent=`${{i+1}} / ${{rows.length}} · frame ${{r.frame_index}} · ${{r.reviewed?'REVIEWED':'UNREVIEWED'}}`;renderBEV(r);document.getElementById('pred').textContent=JSON.stringify(r.prediction||{{}},null,2);let html=''; for(const [path,values] of Object.entries(defs)){{html+=`<div class="group"><h3>${{label(path)}}</h3>`; for(const v of values){{let val=v===null?null:v, cur=get(r.gt,path), pred=get(r.prediction||{{}},path); let c=(cur===val?' active':'')+(pred===val?' pred':''); html+=`<button class="${{c}}" onclick='pick(${{JSON.stringify(path)}},${{JSON.stringify(val)}})'>${{val===null?'none':val}}</button>`;}} html+='</div>';}} html+='<div class="group interaction"><h3>interaction tags</h3>'; for(const t of interactionTags){{let checked=r.gt.interaction_tags.includes(t)?'checked':''; let p=(r.prediction?.interaction_tags||[]).includes(t)?' ★pred':''; html+=`<label><input type="checkbox" ${{checked}} onchange='toggleInteraction(${{JSON.stringify(t)}},this.checked)'> ${{t}}${{p}}</label>`;}} html+='</div>'; document.getElementById('form').innerHTML=html;}}
