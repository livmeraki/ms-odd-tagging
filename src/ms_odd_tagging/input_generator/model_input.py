...
def compact_ld_context(window):
    """Keep descriptive LD availability in model inputs without injecting rule labels."""
    series = []
    for frame in window.get("frames", [])[::5]:
        ld = frame.get("ld", {})
        series.append({
            "frame_index": frame.get("frame_index"),
            "available": ld.get("available") is True,
            "summary": ld.get("summary", {}),
            "quality_flags": ld.get("quality_flags", {}),
        })
    available_rows = [row for row in series if row["available"]]
    return {
        "available": bool(available_rows),
        "sampled_frame_count": len(series),
        "available_sampled_frame_count": len(available_rows),
    }, series
...
def build_refined_json(
    ...
    ld_summary, ld_series_sampled = compact_ld_context(window)
    refined = {
...
Generated by `python -m ms_odd_tagging.input_generator.model_input`.
