...
def ld_context_candidate(frames):
    """Summarize usable lane context without treating LD presence as proof of lane following."""
    available = any(frame.get("ld", {}).get("available") is True for frame in frames)
    qualifying = []
    line_distances = []
    lane_distances = []
    for frame in frames:
        summary = frame.get("ld", {}).get("summary", {})
        has_context = (
            summary.get("nearby_lane_line_count", 0) > 0
            or summary.get("nearby_lane_count", 0) > 0
        )
        qualifying.append(has_context)
        if isinstance(summary.get("nearest_lane_line_distance_m"), (int, float)):
            line_distances.append(summary["nearest_lane_line_distance_m"])
        if isinstance(summary.get("nearest_lane_distance_m"), (int, float)):
            lane_distances.append(summary["nearest_lane_distance_m"])
    fraction = sum(qualifying) / len(frames) if frames else 0.0
    candidate = None
    if available and fraction >= 0.6:
        candidate = {
            "qualifying_fraction": round(fraction, 4),
            "minimum_nearest_lane_line_distance_m": min(line_distances) if line_distances else None,
            "minimum_nearest_lane_distance_m": min(lane_distances) if lane_distances else None,
        }
    return {"available": available, "candidate": candidate}
...
def build_candidates(window_frames, all_turn_events, all_stop_events, frame_interval):
    ...
    ld_context = ld_context_candidate(window_frames)
    ...
    if ld_context["available"] and ld_context["candidate"] is None:
        with_lead = None
        without_lead = None
    elif ld_context["candidate"] is not None:
        if with_lead:
            with_lead["ld_lane_context"] = ld_context["candidate"]
        if without_lead:
            without_lead["ld_lane_context"] = ld_context["candidate"]
    if with_lead:
