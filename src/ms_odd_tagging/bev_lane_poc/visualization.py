"""Minimal BEV-space HTML overlay for the BEV lane POC."""
...
<title>BEV Lane POC</title><style>
:root{--bg:#f8fafc;--fg:#172033;--panel:#fff;--grid:#dbe3ef;--ego:#16a34a;--left:#0284c7;--right:#d97706;--candidate:#94a3b8}
...
.tools{display:flex;gap:12px;align-items:center;flex-wrap:wrap}input[type=range]{flex:1;min-width:300px}canvas{width:100%;background:var(--panel);border:1px solid var(--grid);border-radius:8px;margin-top:12px}
...
</style></head><body><main><h1>BEV Lane POC - <span id="recording"></span></h1>
...
function screen(p,f){const e=f.bev_extent_m,scale=Math.min(canvas.width/(e.left+e.right),canvas.height/(e.back+e.forward)),cx=canvas.width/2,cy=e.forward*scale;return[cx-p[1]*scale,cy-p[0]*scale]}
function polygon(points,f,color,alpha,width){if(!points||points.length<3)return;ctx.beginPath();points.forEach((p,i)=>{const q=screen(p,f);i?ctx.lineTo(...q):ctx.moveTo(...q)});ctx.closePath();ctx.globalAlpha=alpha;ctx.fillStyle=color;ctx.strokeStyle=color;ctx.lineWidth=width;ctx.fill();ctx.stroke();ctx.globalAlpha=1}
function draw(){const f=frames[+slider.value];ctx.clearRect(0,0,canvas.width,canvas.height);ctx.strokeStyle=css('--grid');ctx.lineWidth=1;const e=f.bev_extent_m;for(let y=-e.right;y<=e.left;y+=10){let a=screen([-e.back,y],f),b=screen([e.forward,y],f);ctx.beginPath();ctx.moveTo(...a);ctx.lineTo(...b);ctx.stroke()}for(let x=-e.back;x<=e.forward;x+=10){let a=screen([x,-e.right],f),b=screen([x,e.left],f);ctx.beginPath();ctx.moveTo(...a);ctx.lineTo(...b);ctx.stroke()}
