"""Run the optional BEV lane-detection proof of concept."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--enable-bev-lane-poc", action="store_true", help="Required feature flag. Without it the command performs no work.")
    parser.add_argument("--canonical-dir", type=Path, default=CANONICAL)
    parser.add_argument("--output-root", type=Path, default=Path("outputs/bev_lane_poc"))
    parser.add_argument("--config", type=Path, default=Path("configs/bev_lane_poc.json"))
    parser.add_argument(
        "--frame",
        action="append",
        default=[],
    ...
    if not args.enable_bev_lane_poc:
        print("BEV lane POC disabled; pass --enable-bev-lane-poc to run it.")
        return 0
    config = load_config(args.config if args.config.is_file() else None, {"feature_enabled": True})
    if args.recordings:
        inputs = [
    ...
    for source_path in inputs:
        ...
        if log_path.exists():
            log_path.unlink()
        result = run_recording(recording, config, frame_indices=selected, log=jsonl_logger(log_path))
        output = args.output_root / "results" / f"{recording_id}_bev_lane_poc.json"
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps(result, ensure_ascii=True, indent=2), encoding="utf-8")
        print(f"Wrote {output}")
        print(f"Wrote {log_path}")
        if args.visualize:
            html = args.output_root / "visualization" / f"{recording_id}_bev_lane_poc.html"
