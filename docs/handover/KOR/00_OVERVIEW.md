...
현재 repository의 기본 입력 생성 흐름은 다음과 같다.
...
5. 근거가 부족한 label은 억지로 추론하지 않고 unknown/unsupported로 남긴다.
...
| 04 | `04_SCENARIO_STATUS.md` | scenario별 구현 상태와 신뢰 수준 |
...
**Pipeline → Data Format → Algorithms → Evaluation**
